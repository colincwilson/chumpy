...
class Ch(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        for name, default_val in cls._default_kwargs.items():
            object.__setattr__(result, '_%s' % name,
                               kwargs.get(name, default_val))
            if name in kwargs:
                del kwargs[name]
        ...
        defs = {
            ...
            for p in cls.parm_declarations() if p.default is not None
        }
        defs.update(kwargs)
        result.set(**defs)
    ...
    def parm_declarations(cls):
        if cls.__name__ not in cls._cached_parms:
            ...
            for name, decl in sorted(parameters,
    ...
    @property
    def short_name(self):
        return self.label if hasattr(self,
                                     'label') else self.__class__.__name__
    ...
    def sid(self):
        ...
        return (self.__class__,
                tuple([(k, id(self.__dict__[k])) for k in pnames
                       if k in self.__dict__]))

    def reshape(self, *args):
        return reordering.reshape(a=self,
                                  newshape=args if len(args) > 1 else args[0])
    ...
    def print_labeled_residuals(self,
                                print_newline=True,
                                num_decimals=2,
                                where_to_print=None):
        ...
        if hasattr(self, 'label'):
            where_to_print.write(('%s: %.' + str(num_decimals) + 'e | ') %
                                 (self.label, np.sum(self.r**2)))
        for dterm in self.dterms:
            dt = getattr(self, dterm)
            if hasattr(dt, 'dterms'):
                dt.print_labeled_residuals(print_newline=False,
                                           where_to_print=where_to_print)
        if print_newline:
    ...
    def clear_cache_wrt(self, wrt, itr=None):
        ...
        if itr is None or itr != self._itr:
            for parent, parent_dict in self._parents.items():
                if wrt in parent._cache['drs'] or (hasattr(
                        parent, 'dr_cached') and wrt in parent.dr_cached):
    ...
    def replace(self, old, new):
        ...
        for term_name in [
                t for t in list(self.dterms) + list(self.terms)
                if hasattr(self, t)
        ]:
    ...
    def is_dr_wrt(self, wrt):
        if type(self) == Ch:
            return wrt is self
        dterms_we_have = [
            getattr(self, dterm) for dterm in self.dterms
            if hasattr(self, dterm)
        ]
        return wrt in dterms_we_have or any(
            [d.is_dr_wrt(wrt) for d in dterms_we_have])
        ...
        # minimize_dogleg(obj=self.__getitem__(key) - value, free_variables=kids, show_residuals=False)
            ...
            while not inner.is_ch_baseclass():
                if issubclass(inner.__class__, reordering.Permute):
                    ...
                    raise Exception(
                        "Can't set array that is function of arrays.")
            ...
            inner.x = inner.x + t1.reshape(inner_shape) - t2.reshape(
                inner_shape)
    ...
    def r(self):
        ...
        if self._cache['r'] is None:
            self._cache['r'] = np.asarray(np.atleast_1d(self.compute_r()),
                                          dtype=np.float64,
                                          order='C')
    ...
    def compute_lop(self, wrt, lhs):
        ...
        return self._superdot(
            lhs, dr) if not isinstance(lhs, LinearOperator) else lhs.matmat(dr)
        ...
        def string_for(self, my_name):
            ...
            for dterm in self.dterms:
                if hasattr(self, dterm):
                    dtval = getattr(self, dterm)
                    if hasattr(dtval, 'dterms') or hasattr(dtval, 'terms'):
                        child_label = getattr(dtval, 'label') if hasattr(
                            dtval, 'label') else dterm
                        child_label = '%s (%s)' % (
                            child_label, str(dtval.__class__.__name__))
                        src = 'aaa%d' % (id(self))
                        dst = 'aaa%d' % (id(dtval))

                        s = ''
                        color = color_mapping[dtval._status] if hasattr(
                            dtval, '_status') else 'grey'
                        if dtval == current_node:
                            color = 'blue'
                        if isinstance(dtval, reordering.Concatenate) and len(
                                dtval.dr_cached) > 0:
                            s = 'dr_cached\n'
        ...
        with open(dot_file_name, 'w') as dot_file:
            with open(png_file_name, 'w') as png_file:
                ...
                png_file = tempfile.NamedTemporaryFile(suffix='.png',
                                                       delete=False)
                subprocess.call(
                    ['dot', '-Tpng', '-o', png_file.name, dot_file.name])
    ...
    def show_tree(self, cachelim=np.inf):
        ...
        def string_for(self, my_name):
            ...
            if cachelim != np.inf and hasattr(
                    self, '_cache') and 'drs' in self._cache:
    ...
    def __neg__(self):
        return ch_ops.negative(self)

    def __add__(self, other):
        return ch_ops.add(a=self, b=other)

    def __radd__(self, other):
        return ch_ops.add(a=other, b=self)

    def __sub__(self, other):
        return ch_ops.subtract(a=self, b=other)

    def __rsub__(self, other):
        return ch_ops.subtract(a=other, b=self)

    def __mul__(self, other):
        return ch_ops.multiply(a=self, b=other)

    def __rmul__(self, other):
        return ch_ops.multiply(a=other, b=self)

    def __div__(self, other):
        return ch_ops.divide(x1=self, x2=other)

    def __rdiv__(self, other):
        return ch_ops.divide(x1=other, x2=self)

    def __pow__(self, other):
        return ch_ops.power(x=self, pow=other)

    def __rpow__(self, other):
        return ch_ops.power(x=other, pow=self)

    def __rand__(self, other):
        return self.__and__(other)

    def __abs__(self):
        return ch_ops.abs(self)

    def __gt__(self, other):
        return ch_ops.greater(self, other)

    def __ge__(self, other):
        return ch_ops.greater_equal(self, other)

    def __lt__(self, other):
        return ch_ops.less(self, other)

    def __le__(self, other):
        return ch_ops.less_equal(self, other)

    def __ne__(self, other):
        return ch_ops.not_equal(self, other)
...
def depends_on(*dependencies):
    ...
    def _depends_on(func):
        want_out = 'out' in inspect.getargs(func).args
...
class ChLambda(Ch):
    ...
    def __init__(self, lmb, initial_args=None):
        args = {
            ...
            for idx, argname in enumerate(inspect.getargs(lmb)[0])
        }

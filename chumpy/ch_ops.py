...
__all__ = [
    'array', 'amax', 'amin', 'max', 'min', 'maximum', 'minimum', 'nanmax', 'nanmin',
    'sum', 'exp', 'log', 'mean', 'std', 'var',
    'sin', 'cos', 'tan', 'arcsin', 'arccos', 'arctan',
    'sqrt', 'square', 'absolute', 'abs', 'clip',
    'power', 'add', 'divide', 'multiply', 'negative', 'subtract', 'reciprocal',
    'nan_to_num', 'dot', 'cumsum',
    'floor', 'ceil', 'greater', 'greater_equal', 'less', 'less_equal', 'equal', 'not_equal',
    'nonzero', 'ascontiguousarray', 'asfarray', 'arange', 'asarray', 'copy',
    'cross', 'shape', 'sign', 'SumOfSquares', 'NanDivide'
]
...
numpy_array_creation_routines = [
    ...
    'array', 'arange', 'linspace', 'logspace', 'meshgrid', 'mgrid', 'ogrid',
    'fromfunction', 'fromiter', 'meshgrid', 'tri'
]
wont_implement = [
    'asanyarray', 'asmatrix', 'frombuffer', 'copy', 'fromfile',
    'fromstring', 'loadtxt', 'copyto', 'asmatrix',
    'asfortranarray', 'asscalar', 'require'
]
not_yet_implemented = ['tril', 'triu', 'vander']


__all__.extend(not_yet_implemented)
__all__.extend(wont_implement)
__all__.extend(numpy_array_creation_routines)
__all__.extend(['pi', 'set_printoptions'])
...
for rtn in ['argmax', 'nanargmax', 'argmin', 'nanargmin']:
    exec('def %s(a, axis=None): return np.%s(a.r, axis) if hasattr(a, "compute_r") else np.%s(a, axis)' % (
        rtn, rtn, rtn))
    __all__.append(rtn)


for rtn in ['argwhere', 'nonzero', 'flatnonzero']:
    exec('def %s(a) : return np.%s(a.r) if hasattr(a, "compute_r") else np.%s(a)' % (
        rtn, rtn, rtn))
    __all__.append(rtn)
...
class Cross(ch.Ch):
    ...
    def compute_r(self):
        return np.cross(
            self.a.r, self.b.r, self.axisa,
            self.axisb, self.axisc, self.axis)
...
class cumsum(ch.Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        result = sp.csc_matrix(
            (data, (IS, JS)), shape=(self.a.size, self.a.size))
        return result
...
class UnaryElemwise(ch.Ch):
    ...
    def compute_dr_wrt(self, wrt):
        if wrt is self.x:
            ...
            return sp.diags(
                [result.ravel()], [0]
            ) if len(result) > 1 else np.atleast_2d(result)


class nan_to_num(UnaryElemwise):

    def _r(self, x): return np.nan_to_num(x)

    def _d(self, x): return np.asarray(np.isfinite(x), np.float64)
...
class reciprocal(UnaryElemwise):
    ...
    def _d(self, x): return -np.reciprocal(np.square(x))
...
class sqrt(UnaryElemwise):
    ...
    def _d(self, x): return .5 * my_power(x, -0.5)
...
class arcsin(UnaryElemwise):
    ...
    def _d(self, x): return np.reciprocal(np.sqrt(1. - np.square(x)))
...
class tan(UnaryElemwise):
    ...
    def _d(self, x): return np.reciprocal(np.cos(x)**2.)
...
class arctan(UnaryElemwise):
    ...
    def _d(self, x): return np.reciprocal(np.square(x) + 1.)
...
class negative(UnaryElemwise):
    ...
    def _d(self, x): return np.negative(np.ones_like(x))


class absolute(UnaryElemwise):
    _r = np.abs

    def _d(self, x): return (x > 0) * 2 - 1.
...
class clip(ch.Ch):
    ...
    def compute_dr_wrt(self, wrt):
        if wrt is self.a:
            result = np.asarray((self.r != self.a_min) & (
                self.r != self.a_max), np.float64)
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if self.axis == None:
            ...
            if uid not in self.dr_cache:
                ...
                result = sp.csc_matrix(
                    (data, (idxs_postsum.ravel(), idxs_presum.ravel())), (self.r.size, wrt.size))
...
def _broadcast_matrix(a, b, wrt, data):
    ...
    if len(set((a.shape, b.shape))) == 1:
        ...
        if uid not in _bs_setup_data2:
            ...
            a2 = np.arange(a.size).reshape(
                a.shape) if wrt is a else np.zeros(a.shape)
            b2 = np.arange(b.size).reshape(b.shape) if (
                wrt is b and wrt is not a) else np.zeros(b.shape)
...
def broadcast_shape(a_shape, b_shape):
    ...
    if uid not in broadcast_shape_cache:
        ...
        if False:
            numpy_result = np.broadcast(
                np.empty(a_shape), np.empty(b_shape)).shape
...
class A_extremum(ch.Ch):
    ...
    def compute_dr_wrt(self, wrt):
        if wrt is self.a:
            ...
            JS = np.asarray(np.round(
                mn + stride * self.argf(self.a.r, axis=self.axis)), dtype=np.uint32).ravel()
...
class Extremum(ch.Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.a:
            whichmax = (self.r == self.f(
                self.a.r, self.b.r - self.f(1, -1))).ravel()
        else:
            whichmax = (self.r == self.f(
                self.b.r, self.a.r - self.f(1, -1))).ravel()
        IS = IS[whichmax]
        JS = JS[whichmax]
        data = np.ones(JS.size)
...
class multiply(ch.Ch):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        b2 = self.b.r if (
            wrt is self.a and wrt is not self.b) else np.ones(self.b.shape)
        data = (a2 * b2).ravel()
